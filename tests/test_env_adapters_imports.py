...
def test_libero_adapter_uses_fallback_when_lerobot_processor_dependency_missing():
    script = r'''
import builtins
import sys
import types
...
def test_libero_init_states_fall_back_to_installed_package_when_config_path_is_stale(tmp_path):
    configured_root = tmp_path / "stale_config_root"
    package_root = tmp_path / "installed_libero"
    init_dir = package_root / "init_files" / "libero_object"
...
configured_root = "__CONFIGURED_ROOT__"
package_root = Path("__PACKAGE_ROOT__")
torch.save({"loaded_from": "installed_package"}, package_root / "init_files" / "libero_object" / "demo.pruned_init")
...
libero = types.ModuleType("libero")
libero_libero = types.ModuleType("libero.libero")
libero_libero.__file__ = str(package_root / "__init__.py")
libero_libero.benchmark = types.SimpleNamespace(get_benchmark_dict=lambda: {})
libero_libero.get_libero_path = lambda name: configured_root
libero_envs = types.ModuleType("libero.libero.envs")
libero_envs.OffScreenRenderEnv = object
sys.modules["libero"] = libero
sys.modules["libero.libero"] = libero_libero
...
task = types.SimpleNamespace(problem_folder="libero_object", init_states_file="demo.pruned_init")
suite = types.SimpleNamespace(tasks=[task])

loaded = module.get_task_init_states(suite, 0)
assert loaded == {"loaded_from": "installed_package"}
'''.replace("__ADAPTER_PATH__", str(Path(__file__).resolve().parents[1] / "core" / "env_adapters" / "libero_adapter.py"))
    script = script.replace("__CONFIGURED_ROOT__", str(configured_root))
    script = script.replace("__PACKAGE_ROOT__", str(package_root))
