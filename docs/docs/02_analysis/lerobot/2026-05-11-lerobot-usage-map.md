...
3. **Observation pipeline glue inside LiberoAdapter** — `LiberoAdapter` instantiates `lerobot.processor.pipeline.PolicyProcessorPipeline` with `lerobot.processor.env_processor.LiberoProcessorStep` to convert MuJoCo-side observations into LeRobot-canonical format.
...
| [`main.py`](../../../../main.py#L84) | 84 | `from lerobot.envs.factory import make_env_pre_post_processors` | ❌ **DEAD IMPORT** | Never invoked. |
...
| [`core/env_adapters/libero_adapter.py`](../../../../core/env_adapters/libero_adapter.py#L37) | 37 | `from lerobot.processor.env_processor import LiberoProcessorStep` | ✅ FACT | `env_preprocessor_steps.append(LiberoProcessorStep())` at line 753. The only step actually installed today. |
...
- `OBS_ENV_STATE = "observation.environment_state"` (**INFERENCE** — not directly read in this round, but standard LeRobot convention)
- `ACTION = "action"`
...
**Where they fire:**
- `env_preprocessor`:
  - `obs = self.env_preprocessor(obs)` at [`libero_adapter.py:848`](../../../../core/env_adapters/libero_adapter.py#L848) inside `get_policy_observation`
- `env_postprocessor`:
  - `action_transition = self.env_postprocessor(action_transition)` at [`libero_adapter.py:1425`](../../../../core/env_adapters/libero_adapter.py#L1425) inside `step()` — currently empty (no postprocessor steps registered), so a no-op
...
- `core.env_adapters.BaseEnvAdapter` (project-internal abstraction)
