...
**FACT — only the in-tree variant matters at runtime.** `LiberoAdapter._env[idx]` is built from `OffScreenRenderEnv(**env_args)` at `libero_adapter.py:540`, and that `OffScreenRenderEnv` resolves to `libero.libero.envs.OffScreenRenderEnv` from the conda site-packages.
