...
4. Runs `self.env_preprocessor(obs)`.
...
1. Applies `self.env_postprocessor`, currently an empty `PolicyProcessorPipeline`.
2. Clones the tensor.
...
```bash
python - <<'PY'
import core.env_adapters.libero_adapter as la
