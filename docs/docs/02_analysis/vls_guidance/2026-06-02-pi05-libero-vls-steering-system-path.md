...
Main._run_episode
  ...
  -> loop:
     ...
     -> optional adapter.env_postprocessor(action_chunk)
...
`Main._run_episode()` then optionally applies `adapter.env_postprocessor` to the whole chunk (`main.py:680-683`). In the LIBERO adapter as inspected, `env_postprocessor` is an empty pipeline (`core/env_adapters/libero_adapter.py:861-865`), so this is expected to be a no-op unless processor steps are later added.
